...
export default function CombinedPanel({
  ...
  const title = selectedNode.name || selectedNode.stage_name || selectedNode.id;
    ...
    >
      ...
      {songs.length > 0 && (
        <div>
          ...
          <ul
            style={{ margin: "4px 0 8px 16px", padding: 0, listStyle: "disc" }}
          >
            {songs.map((s) => (
              <li
                key={s.id}
      ...
      {selectedNode.connectionPath && (
        <div style={{ marginTop: "1rem" }}>
          <strong style={{ color: "#ccc" }}>
            Sailor Shift ← {selectedNode.name}
          </strong>
          <div
            style={{
              marginTop: "0.5rem",
          ...
          >
            {selectedNode.connectionPath.map((step, i) => (
              <div key={i}>→ {step}</div>
            ))}
          </div>
        </div>
