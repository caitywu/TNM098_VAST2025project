...
export default function Graph({
  ...
  useEffect(() => {
    ...
    const edges = container
      ...
      .data(
        linkData,
        (d, i) => `${d.source.id}-${d.target.id}-${d["Edge Type"]}-${i}`
      )
      .join("path")
      .attr("fill", "none")
      .attr("stroke-width", 1)
      .attr(
        "stroke",
        (d) => `url(#grad-${d.source.id}-${d.target.id}-${d["Edge Type"]})`
      )
      .style("opacity", 0.6)
      .on("mouseover", (e, d) => {
    ...
    function ticked() {
      ...
      linkData.forEach((d) => {
        svg
          .select(`#grad-${d.source.id}-${d.target.id}-${d["Edge Type"]}`)
          .attr("x1", d.source.x)
          .attr("y1", d.source.y)
          .attr("x2", d.target.x)
          .attr("y2", d.target.y);
